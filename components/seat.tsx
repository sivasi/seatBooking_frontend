...
const SeatBooking = () => {
  ...
  const handleBooking = async (event: React.FormEvent) => {
    ...
    // time complexity is O(n^3) where n = Maximum of (ROWS, COLS)
    ...
    <>
      <div className='container mx-auto'>
        {(showToast || showToast1) && (
          <div
            id="toast-success"
            className="m-4 fixed left-1/2 top-4 transform -translate-x-1/2 flex items-center justify-center w-60 max-w-xs p-4 text-gray-500 bg-white rounded-lg shadow-sm dark:text-gray-400 dark:bg-gray-800 z-50"
            role="alert"
          >
        ...
        <div className='m-4 flex justify-end'>
          <button onClick={handleLogout} className="disabled:opacity-50 text-[12px] w-15 h-8 text-white bg-pink-600 cursor-pointer hover:bg-pink-700 focus:ring-4 focus:outline-none focus:ring-pink-300 font-medium rounded-lg text-center dark:bg-pink-600 dark:hover:bg-pink-700 dark:focus:ring-pink-800">
            <div className=" flex justify-center items-center">Log out</div>
          </button>
        </div>  
        <div className='m-4 flex justify-between max-[1024px]:block'>
          <div className='max-[1024px]:flex max-[1024px]:justify-center'> 
          <div className='bg-white dark:bg-gray-800 p-4 rounded-xl shadow-md w-120 max-[480px]:w-87'>
            <h2 className="text-xl dark:text-white flex justify-center font-semibold mb-4">Seat Ticket</h2>
            <div className="grid grid-cols-7 gap-x-1 gap-y-4">
                {seats.map((seat) => (
                  <div
                    key={seat.seat_number}
                    className={`rounded-xl flex items-center justify-center h-9 w-full font-bold text-lg transition
            ...
            <div className='flex justify-between py-3 h-16'>
              <div className='bg-yellow-400 dark:bg-yellow-600 rounded-xl text-l max-[480px]:text-sm font-bold flex justify-center items-center w-50 max-[480px]:w-36'>Booked Seats = {bookedCount}</div>
              <div className='bg-lime-400 dark:bg-lime-600 rounded-xl text-l max-[480px]:text-sm font-bold flex justify-center items-center w-50 max-[480px]:w-36'>Available Seats = {availableCount}</div>
            </div>
          </div>
        </div>
        <div className='w-full flex justify-center items-center max-[1024px]:mt-30 min-[1024px]:justify-end'>
          <div className=''>
            <div className='flex'>
